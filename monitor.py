...
# Config
# ---------------------------------------------------------------------------
REVIEW_CHECK_INTERVAL_MINUTES = int(
    os.getenv("REVIEW_CHECK_INTERVAL_MINUTES", "60")
)
...
async def process_single_review(
    ...
    logger.info("Generating reply for new review: %s (rating=%d)", review_id, rating_int)
    try:
...
async def run_sync_once() -> dict:
    ...
    summary = {"locations_checked": 0, "reviews_found": 0, "new_replies_posted": 0, "errors": 0}
